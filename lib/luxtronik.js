...
Luxtronik.prototype._processData = function (result,callback) {
    ...
    const command = result.responses.find( response => response.command === 3005 );
    for (let i = 0; i<result.responses.length; i++) {
        ...
        let swappedMap;
        let mapped = typeConfiguration.dataTypes[result.responses[i].command].mapping.getMappedValues(result.responses[i].payload);
...
Luxtronik.prototype._processDatas = function () {
    if (this.connection.action === 'read') {
        //return this.connection.callback(null, this.connection[3003]);

        const heatpumpParameters = utils.toInt32ArrayReadBE(this.connection['3003'][this.connection.sendData[1]].payload);
        const heatpumpValues = utils.toInt32ArrayReadBE(this.connection['3004'][this.connection.sendData[1]].payload);
        const heatpumpVisibility = this.connection['3005'][this.connection.sendData[1]].payload;

        if (typeof heatpumpParameters === 'undefined' ||
            typeof heatpumpValues === 'undefined' ||
            typeof heatpumpVisibility === 'undefined') {
            return this.connection.callback(new Error('Unexpected Data'));
        }
        ...
        const additional = {
            ...
            'reading_calculated_time_ms': this.connection.exectutionEndTime - this.connection.executionStartTime
        };
        ...
        return this.connection.callback(null, {
            values,
            parameters,
            additional
        });
    }
    if (this.connection.action === 'write') {
        return this.connection.callback(null, this.connection[3002]);
    }
};
