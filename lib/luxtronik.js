...
/**
 * Handles the amount of jos for reading data
 */
Luxtronik.prototype._nextJob = function () {
    if (this.receivy.writeParams.length > 0) {
        this.receivy.activeCommand = 0;
        this.receivy.sendData = this.receivy.writeParams.shift()
        sendData(this.client, this.receivy.sendData);
    } else {
        this.client.end();
        this.client = null;
        this.receivy.executionEndTime = Date.now();
...
Luxtronik.prototype.read = function (callback) {
    this.receivy = {
        jobs: this._typeConfiguration.getReadableDataTypes(),
        activeCommand: 0,
        executionStartTime: Date.now(),
        callback
    };
    this.receivy.writeParams = [];
    for (var job in this.receivy.jobs) {
        this.receivy.writeParams.push([this.receivy.jobs[job],0]);
    }
    
    client.createReadClient.bind(this)();
};


/**
 * Writes data to the Heatpump
 * 
...
Luxtronik.prototype._handleWriteCommand = function (parameterName, realValue, callback) {
    ...
    if (typeof set.setParameter !== 'undefined') {
        ...
        client.createWriteClient.bind(this)();
    } else {
...
Luxtronik.prototype.write = function (parameterName, realValue, callback) {
    ...
    this.receivy = {
        jobs: this._typeConfiguration.getWritableDataTypes(),
        activeCommand: 0,
        executionStartTime: Date.now(),
        callback
    };
...
Luxtronik.prototype._processData = function () {
    if (this.receivy.action === "read") {
        //return this.receivy.callback(null, this.receivy[3003]);

        const heatpumpParameters = utils.toInt32ArrayReadBE(this.receivy['3003'][this.receivy.sendData[1]].payload);
        const heatpumpValues = utils.toInt32ArrayReadBE(this.receivy['3004'][this.receivy.sendData[1]].payload);
        const heatpumpVisibility = this.receivy['3005'][this.receivy.sendData[1]].payload;
        ...
        const additional = {
            '3003': typeConfiguration.heatpumpParameter.getRawHeatPumpParameters(heatpumpParameters),
            '3004': typeConfiguration.heatpumpValues.getRawHeatPumpValues(heatpumpValues),
            '3005': typeConfiguration.heatpumpVisibilities.getHeatpumpVisibilities(heatpumpVisibility),
            'reading_calculated_time_ms': this.receivy.exectutionEndTime - this.receivy.executionStartTime
        };
    ...
    if (this.receivy.action === "write") {
        return this.receivy.callback(null, this.receivy[3002])
    }
};
