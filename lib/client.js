...
const createClient = function() {
     ...
     */
    this.client.on('data_s', function (data) {
        logger.log({level: 'verbose',message: 'receiving data'});
        logger.log({level: 'verbose',message: 'Bytes read: ' +this.client.bytesRead});
    ...
    this.client.on('data', function (data) {
        let commandEcho = this.receivy.sendData[0];
        ...
        this.receivy[commandEcho].rawData = Buffer.concat([this.receivy[commandEcho].rawData,data]);

        // if bytes returned > 4
        if (data.length > 4) {
            // read 4bytes sequences until data
            let headersequences = this._typeConfiguration.dataTypes[commandEcho].firstReadableDataAddress;
            let bytelength = this._typeConfiguration.dataTypes[commandEcho].bytelength;
            logger.log({level: 'verbose',message: 'read Headerinformation'});
            for (var i = 0; i < headersequences; i+=4) {
                    let dataEntry = data.readInt32BE(i);
                    logger.log({level: 'debug',message: 'pushed data: ' + dataEntry});
                    this.receivy[commandEcho].readData.push(dataEntry);
                }
            logger.log({level: 'verbose',message: 'read Data'});
            for (var i = headersequences; i < data.length; i+= bytelength) {
                let dataEntry;
            ...
            this.receivy[commandEcho].readData.push(data.readInt32BE(0));
        ...
        let elementOfReturnCounts = this._typeConfiguration.dataTypes[commandEcho].firstByteOfParameterCount / 4;

        if (this.receivy[commandEcho].readData.length > elementOfReturnCounts &&  this.receivy[commandEcho].readData.length === this.receivy[commandEcho].readData[elementOfReturnCounts] + elementOfReturnCounts + 1) {
            process.nextTick(this._nextJob.bind(this));
        }
}.bind(this));
}
